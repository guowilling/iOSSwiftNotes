...
extension UIWindow {
    ...
    static func showTips(text: String) {
        ...
        let tapGesture = UITapGestureRecognizer.init(target: self, action: #selector(handlerTapGuesture(sender:)))
    ...
    @objc static func handlerTapGuesture(sender: UIGestureRecognizer) {
        dismiss()
    ...
    @objc static func dismiss() {
        if let tapGesture = tap {
            (UIApplication.shared.delegate?.window as? UIWindow)?.removeGestureRecognizer(tapGesture)
        }
