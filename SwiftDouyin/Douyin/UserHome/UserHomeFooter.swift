...
protocol UserHomeFooterDelegate: NSObjectProtocol {
    func userHomeFooter(_ footer: UserHomeFooter, onTapIndex index: Int)
}

class UserHomeFooter: UICollectionReusableView {
    
    var delegate: UserHomeFooterDelegate?
    ...
    @objc func onTapAction(sender:UITapGestureRecognizer) {
        ...
        if delegate != nil {
            UIView.animate(withDuration: 0.1, delay: 0.0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0.0, options: .curveEaseInOut, animations: {
                ...
                self.delegate?.userHomeFooter(self, onTapIndex: index)
            }
        }
    }
}
