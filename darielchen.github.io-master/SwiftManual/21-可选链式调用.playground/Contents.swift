...
//如果没有值,那么调用失败.也可以是多个调用连接在一起,如果其中有一个节点为nil,整个调用链都会调用失败,返回nil
...
if let roomCount = p1.home?.numberOfRooms { // 使用问好(?)来替代感叹号(!)
    ...
    print("roomCount为nil")             // roomCount为nil
}

p1.home = Home()                       // p1.home不为空
...
// 为可选链式调用定义模型类
// 通过可选链式可以调用多层属性,方法和下标,并且能够判断这些子属性,方法或下标是否可以访问.下面定义了四个模型类
class People {
...
class Address {
    var buildingName: String?            // 建筑名称
    var buildingNumber: String?          // 建筑号码
    var street: String?
    func buildingIdentifier() -> String? { // 根据情况返回建筑的标识
        if buildingName != nil {
            return buildingName
...
// 通过可选链式调用访问属性
let jone = People()
if let roomCount = jone.residence?.numberOfRooms { // 可选绑定
    print(roomCount)
} else {
    print("jone.residence为nil")         // numberOfRooms为nil,可选链式调用失败
}
...
// 多层可选链式调用:通过可选链式调用非可选类型,将会返回一个可选类型,通过可选链式调用可选类型,返回的依然还是可选类型
if let joneStreet = jone.residence?.address?.street { // 当有一个属性为nil时调用失败
    print(joneStreet)                  // joneStreet
}

// 在方法的可选返回值上进行可选链式调用: 直接在方法的返回值上进行可选链式调用,只要加一个问好(?)
