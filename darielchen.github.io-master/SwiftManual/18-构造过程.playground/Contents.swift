...
let originRect = Rect(origin: RectPoint(x: 2.0, y: 2.0), size: RectSize(width: 2.0, height: 2.0))                 // 将size和origin的参数赋值给对应的存储属性

let centerRect = Rect(center: RectPoint(x: 3.0, y: 3.0), size: RectSize(width: 2.0, height: 2.0))                 // 先计算origin坐标,再在内部调用其他构造器进行赋值
